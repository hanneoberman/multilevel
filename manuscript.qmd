...
M1: model with explanatory variables
```{r}
fit <- lmer(
...
M1A: model with explanatory variables, extraversion slope random
```{r}
fit <- lmer(
...
M0: model with with cross-level interaction
```{r}
fit <- lmer(
  popularity_ij ~ gender_ij + extraversion_ij + experience_j + 
...
The last model does not converge. This is likely due to the missingness in the data. We will now impute the incomplete data and re-fit the models.
...
All incomplete variable have a multilevel imputation method.
...
There are no cluster-level interactions required either, but there is a cross-level interaction implied by the analytic model. We will use passive imputation to impute the interaction effect for extraversion and experience. This requires a differentiation between the cluster means of extraversion and each unit's deviation from the cluster mean.

We add an empty column to the data for the cluster means of extraversion and inform `mice` to impute this variable on the fly as the cluster mean using the `miceadds` method `2l.groupmean`. 
```{r}
dat$extraversion_m <- NA
meth["extraversion_m"] <-  "2l.groupmean"
```

We add an empty column to the data for the unit-level deviations of extraversion and let `mice` derive the deviations from the cluster mean by means of passive imputation.
```{r}
dat$extraversion_d <- NA
meth["extraversion_d"] <- "~ I(extraversion_ij - extraversion_m)"
```

Now we may include the interaction term to the data and methods vector. 
```{r}
dat$extraversion_experience <- NA
meth["extraversion_experience"] <- "~ I(extraversion_d * experience_j)"
```

Now, let's add these three additional variables to the predictor matrix and add the cluster identifier as predictor for these variables.
```{r}
pred <- rbind(
  pred, 
  extraversion_m = 0, 
...
pred[, "cluster_id"] <- -2
# pred["extraversion_m", "extraversion_ij"] <- 1
```

Edit the predictor matrix so the interaction term is included in the imputation model for 'popularity'.
```{r}
pred["popularity_ij", "extraversion_experience"] <- 1
```

Finally, include a predictor related to the missingness and the target: the auxiliary variable teacher assessment. 
```{r}
...
Our final predictor matrix can be visualized as follows.
```{r}
...
This code runs! Evaluate the logged events.
```{r}
imp$loggedEvents
```

Apparently, the extraversion cluster means...
```{r}
imp <- mice.mids(imp, maxit = 9)
plot_pattern(complete(imp))
```

The imputation process has been successful. We can now evaluate the imputations and pool the results for further analysis.
...
<!-- 1. Include all variables that appear in the analysis model, including transformation, interactions and outcomes -->
