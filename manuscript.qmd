...
We can use `2l.norm`: a multilevel method using the linear mixed model with heterogeneous error variances. The unit-level predictor variable 'gender' is dichotomous. 
```{r}
ggmice(dat, aes(as.factor(gender_ij))) + 
  geom_bar(position = "stack", fill = "white")
```

We can use `2l.bin` to impute the missing values. The unit-level predictor variable 'extraversion' is approximately normally distributed, but not truly continuous. 
```{r}
ggmice(dat, aes(extraversion_ij)) + 
  geom_histogram(fill = "white")
```

We can use `2l.pmm` to impute the missing values. The unit-level auxiliary variable 'teacher assessment' is approximately normally distributed but not continuous either. We will use `2l.pmm` for this variable too.
...
The cluster-level variable 'teacher experience' is continuous. We can use `2lonly.mean` to impute the missing values if we are sufficiently certain that this variable... Otherwise, `2lonly.pmm` is a good alternative.
